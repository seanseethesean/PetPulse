...
describe("AuthService", () => {
  ...
  describe("signUpUser", () => {
    it("creates a new user and Firestore doc if not exists", async () => {
      mockCreateUser.mockResolvedValue({ user: { uid: fakeUID, email: fakeEmail } });
    ...
    it("handles Firebase auth/email-already-exists error", async () => {
      ...
      await expect(AuthService.signUpUser(fakeEmail, "password123")).rejects.toEqual({
        status: 400,
  ...
  describe("verifyGoogleToken", () => {
    ...
    beforeEach(() => {
      AuthService.__Rewire__("verifyIdToken", mockVerifyIdToken); // Optional if using rewire
    });
    ...
    it("handles auth/id-token-expired error", async () => {
      ...
      await expect(AuthService.verifyGoogleToken(idToken)).rejects.toEqual({
        status: 401,
